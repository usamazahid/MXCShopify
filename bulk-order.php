...
$orders = shopify_call($token, $shop_url, "/admin/api/2022-07/orders.json", array("fulfillment_status" => "unfulfilled", "ids" => $order_ids ), 'GET');
// echo $orders;
...
<div class="col-lg-12">
<button class="btn btn-primary" id="submit">Send to MXC</button>
<buton class="btn btn-success" id="print" hidden>Print</buton>
</div>
...
<table id="example" class="display" style="width:100%">
    ...
    <tbody>
        ...
        foreach($orders['orders'] as $order)
            {
                echo strpos(strtoupper($order['payment_gateway_names'][0]), 'CASH 1ON DELIVERY');
        ?>
        <tr>
            ...
            <td><input type="text" id="consignment_description" name="consignment_description" value="<?foreach($order['line_items'] as $items){echo $items['name'] . ' (SKU '. $items['sku'] . "), \n"; foreach($items['properties'] as $properties){echo $properties['value'] . ", ";}}?>"></td>
            <td><input type="text" id="amount" name="amount" value="<? echo (strpos(strtoupper($order['payment_gateway_names'][0]), 'CASH ON DELIVERY') !== false)? $order['current_total_price'] : 0 ?>"></td>
